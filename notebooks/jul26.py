...
date_0 = dt.datetime (year = 2016, month = 7, day = 18)
...
init1 = []
for num in plots:
    trajectories1.append(np.loadtxt(directory+"%s/traj.txt"%num, delimiter = ' '))
    init1.append(np.loadtxt(directory+"%s/initial_positions.txt"%num))
    
    trajr = np.loadtxt(directory+"%s/traj.txt"%num, delimiter = ' ')
...
#anim.save("jul26presentation2.mp4", writer=mywriter)
